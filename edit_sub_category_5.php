<?php
    ...
    $db = getDbInstance();
    $db->where('id', $get_id);
    $update_data = $db->getOne('sub_category_5');
    ...
    $db = getDbInstance();
    $db->where('sub_category_id_3', $update_data['sub_category_id_3']);
    ...
    if ($_POST) {
      ...
      $db->where('id',$get_id);
      $resonce = $db->update('sub_category_5',$data);
      header('location: sub_category_5.php');exit;
    }
    ...
    <form action="" method="post">
      <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
        ...
        <label class="block mt-4 text-sm">
          ...
          <select name="sub_category_id_4" class="sub_category_5 block w-full mt-1 text-sm dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 form-select focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray" required>
            <option>Choose a Value</option>
            <?php foreach ($sub_4_categories as $sub_4_category) { ?>
              <option value="<?php echo $sub_4_category['id'] ?>" <?php echo ( $sub_4_category['id'] == $update_data['sub_category_id_4'] ) ? 'selected' : '' ?>><?php echo $sub_4_category['name'] ?></option>
            <?php } ?>
        ...
        <div class="flex mt-6 mb-6 justify-end">
            <div>
              <button class="mr-4 px-10 py-3 font-medium leading-5 text-white transition-colors duration-150 bg-zinc-600 border border-transparent rounded-lg hover:bg-zinc-800 focus:outline-none" onclick="window.location.href='sub_category_5.php'">
                Cancel
              </button>
            </div>
