<?php
    ...
    if($_SERVER['REQUEST_METHOD'] == 'POST'){
        ...
        $mysqli_query = "SELECT * FROM `users` WHERE `email`='".$username."' AND `is_active`=1";
        $execute = mysqli_query($conn, $mysqli_query);

        if(mysqli_num_rows($execute) > 0){
            $user_details = mysqli_fetch_assoc($execute);
                        ...
                        <div class="w-full">
                            <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']);?>" method="post">
                                ...
                                <label class="block text-sm">
                                    ...
                                    <input type="text" name="username" class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input" placeholder="Jane Doe" required/>
                                </label>
                                <label class="block mt-4 text-sm">
                                    <span class="text-gray-700 dark:text-gray-400">Password</span>
                                    <input type="password" name="password" class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input" placeholder="**********" required/>
                                </label>
